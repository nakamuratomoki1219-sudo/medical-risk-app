    ...
    if not api_key:
        ...
        with st.spinner(f"📚 選択された {len(selected_pdfs)} 個のガイドラインを視覚解析し、病態生理を推論中..."):
            try:
                ...
                # ユーザーのご指示通り、Mitral E, A, LVOTの単位をすべてm/sとして送信します！
                ...
                ・Rt-ICA: 径={fmt(rt_ica_diam,'mm')}, Vmax={fmt(rt_ica_vmax)}, Vmin={fmt(rt_ica_vmin)}, Vmean={fmt(rt_ica_vmean)}, PI={fmt(rt_ica_pi)}, RI={rt_ica_ri)}
                ・Lt-ICA: 径={fmt(lt_ica_diam,'mm')}, Vmax={lt_ica_vmax)}, Vmin={fmt(lt_ica_vmin)}, Vmean={fmt(lt_ica_vmean)}, PI={fmt(lt_ica_pi)}, RI={fmt(lt_ica_ri)}
